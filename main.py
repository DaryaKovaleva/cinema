...
class Print:
    ...
    def generate(self):
        ...
        pdf.add_page()
        pdf.image('ticket.png', w = pdf.w/3, h = pdf.h/12)
        pdf.set_font(family = 'Times', size = 16, style = 'B')
        pdf.cell(w = 150, h = 50, txt = f'namefilm: {self.namefilm}', border = 0, align = 'L', ln = 1)
        pdf.cell(w = 150, h = 50, txt = 'row   number', border = 0, align = 'L', ln = 1)
        pdf.cell(w = 150, h = 50, txt = f'{row}          {number}', border = 0, align = 'L', ln = 1)
...
class Question:
    def __init__(self, place):
        ...
        self.myprint1 = tk.Button(self.roott, text='распечатать', font=('Arial', 12), bg='light green', command = self.ticket)
        self.myprint1.grid(row=4, column=1, columnspan=2, sticky='wens', padx=10, pady=10)
        self.myprint2 = tk.Button(self.roott, text='не распечатывать', font=('Arial', 12), bg='light green', command = self.close)
    ...
    def ticket(self):
        ticket = Print('Supermen', self.place)
        ticket.generate()
